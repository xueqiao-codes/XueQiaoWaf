...
namespace BolapanControl.ItemsFilter {
   // <summary>
    ...
    public sealed class FilterPresenter : DependencyObject {
        ...
        private static ICollectionView TryParseSourceCollectionView(IEnumerable source)
        {
            if (source == null)
                return null;
        ...
        public static FilterPresenter TryGet(IEnumerable source){
            ICollectionView sourceCollectionView = TryParseSourceCollectionView(source);
            if (sourceCollectionView == null) return null;
        ...
        /// <returns></returns>
        public static bool TryRemoveFilterPresenter(IEnumerable source)
        {
            ICollectionView sourceCollectionView = TryParseSourceCollectionView(source);
            if (sourceCollectionView != null)
                return filterPresenters.Remove(sourceCollectionView);
            return false;
        }
        ...
        /// <returns></returns>
        public void TryRemoveFilter(Filter filter)
        {
            foreach (var _f in filters)
            {
                _f.Value.Remove(filter);
            }
        }
