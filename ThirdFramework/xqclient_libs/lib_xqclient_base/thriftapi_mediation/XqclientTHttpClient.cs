...
{
    /// <summary>
    /// 雪橇客户端使用的 thrift http transport
    /// </summary>
	public class XqclientTHttpClient : TTransport, IDisposable
	{
        ...
        public XqclientTHttpClient(Uri u)
            : this(u, Enumerable.Empty<X509Certificate>())
        {
        }

        public XqclientTHttpClient(Uri u, IEnumerable<X509Certificate> certificates)
        {
            uri = u;
        ...
        private void SendRequest()
        {
            ...
            {
                var webResp = (HttpWebResponse)wx.Response;
                if (webResp != null)
                {
                    throw new XqclientTHttpException(webResp.StatusCode.GetHashCode(), webResp.StatusDescription, wx);
                }
                else
                {
                    throw new TTransportException(TTransportException.ExceptionType.Unknown, wx.ToString(), wx);
                }
            }
            catch (Exception _ex)
            {
                throw new TTransportException(TTransportException.ExceptionType.Unknown, _ex.ToString(), _ex);
            }
        }
