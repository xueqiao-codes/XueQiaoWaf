...
namespace XueQiaoWaf.Trade.Modules.Applications.ViewModels
{
    ...
    public class ContractListViewModel : ViewModel<IContractListView>
    {
        private readonly SubscribeContractService subscribeContractService;
       
        [ImportingConstructor]
        protected ContractListViewModel(IContractListView view,
            SubscribeContractService subscribeContractService,
            IExchangeDataService exchangeDataService) : base(view)
        {
            ...
            var syncContractItems = new SynchronizingCollection<SubscribeContractDataModel, SubscribeContractDataModel>(subscribeContractService.Contracts, i => i);
            ContractCollectionView = CollectionViewSource.GetDefaultView(syncContractItems) as ListCollectionView;

            ConfigContractCollectionView();
        ...
        public IExchangeDataService ExchangeDataService { get; private set; }

        public ListCollectionView ContractCollectionView { get; private set; }
        ...
        private SelectedContractsOperateCommands selectedItemsOptCommands;
        public SelectedContractsOperateCommands SelectedItemsOptCommands
        {
        ...
        /// cmd param: <see cref="SubscribeContractDataModel"/>类型
        /// </summary>
        ...
        /// <summary>
        /// 点击项目的价格相关列 command
        /// cmd param: 数组类型。arg0:<see cref="SubscribeContractDataModel"/>类型，arg1: <see cref="double"/>类型
        /// </summary>
        ...
        private void ConfigContractCollectionView()
        {
            var listView = ContractCollectionView;
            if (listView == null) return;

            // 合约列表的排序：未过期的在上，过期的在下
            listView.SortDescriptions.Add(new SortDescription { PropertyName = nameof(SubscribeContractDataModel.IsXqTargetExpired), Direction = ListSortDirection.Ascending });
            listView.SortDescriptions.Add(new SortDescription { PropertyName = nameof(SubscribeContractDataModel.AddTimestamp), Direction = ListSortDirection.Ascending });
            listView.LiveSortingProperties.Add(nameof(SubscribeContractDataModel.IsXqTargetExpired));
            listView.LiveSortingProperties.Add(nameof(SubscribeContractDataModel.AddTimestamp));
            listView.LiveSortingProperties.Add(nameof(SubscribeContractDataModel.XqTargetName));
        ...
        private void InvalidateListGlobalFilter()
        {
            ...
            {
                var subItem = obj as SubscribeContractDataModel;
                if (subItem == null) return false;
                if (subItem.SubscribeGroupKey != SubscribeContractDataModel.SharedListContractGroupKey) return false;
                ...
                if (groupTab != null)
                {
                    switch (groupTab.GroupType)
                    {
                        ...
                        case SubscribeDataGroupType.ExistPosition:
                            ...
                            break;
                        case SubscribeDataGroupType.ComposeRelated:
                            isPassGroupTabFilter = subItem.IsComposeRelated;
                            break;
            ...
            {
                nameof(SubscribeContractDataModel.CustomGroupKeys),
                nameof(SubscribeContractDataModel.OnTradingSubAccountIds),
                nameof(SubscribeContractDataModel.ExistPositionSubAccountIds),
                nameof(SubscribeContractDataModel.IsComposeRelated),
                nameof(SubscribeContractDataModel.IsXqTargetExpired)
            };
