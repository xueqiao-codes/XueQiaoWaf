...
namespace XueQiaoWaf.Trade.Modules.Applications.Controllers
{
    ...
    internal class DisplayColumnsOrderListParkedConfigDialogController : IController
    {
        private readonly IMessageWindowService messageWindowService;
        private readonly IEventAggregator eventAggregator;
        private readonly ITradeModuleService tradeModuleService;
        private readonly Lazy<ILoginUserManageService> loginUserManageService;
        private readonly DisplayColumnsOrderListParkedConfigItemsContainerModel configItemsContainerModel;
        ...
        public DisplayColumnsOrderListParkedConfigDialogController(IMessageWindowService messageWindowService,
            IEventAggregator eventAggregator,
            ITradeModuleService tradeModuleService,
            Lazy<ILoginUserManageService> loginUserManageService,
            DisplayColumnsOrderListParkedConfigItemsContainerModel configItemsContainerModel,
        ...
        public void Initialize()
        {
            ...
            dialogContentViewModel.ApplyAsGlobalText = "应用到全部预埋单列表";
        ...
        public void Run()
        {
            configureDialog = messageWindowService.CreateContentCustomWindow(DialogOwner, null, null, true, false, 
                true, "预埋单列表列配置", dialogContentViewModel.View);
        ...
        {
            if (e.PropertyName == nameof(DisplayColumnsOrderListParkedConfigItemsContainerModel.SelectedConfigItem))
            {
                // 更新操作命令合集对象的选中项
        ...
        private void DisplayColumnsConfigItemsReset(IEnumerable<ListColumnInfo> toDisplayColumnInfos)
        {
            ...
            var allColumnsNames = Enum.GetNames(typeof(OrderListColumn_Parked));
            var notDisplayColumnCodes = allColumnsNames
                .Select(n =>
                {
                    Enum.TryParse(n, out OrderListColumn_Parked column);
        ...
        private void SaveConfiguration()
        {
            ...
            if (dialogContentViewModel.IsApplyAsGlobal)
            {
                ...
                if (dataRoot != null)
                {
                    dataRoot.GlobalAppliedOrderListParkedDisplayColumns
                        = configuredColumns.Select(i => (ListColumnInfo)i.Clone()).ToArray();
                }
                // 发布一个事件
                eventAggregator.GetEvent<GlobalApplyOrderListParkedColumnsEvent>().Publish(configuredColumns);
            }
