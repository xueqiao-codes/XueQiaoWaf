...
namespace XueQiaoWaf.Trade.Modules.Applications.Controllers
{
    ...
    internal class DisplayColumnsComposeListConfigDialogController : IController
    {
        private readonly IMessageWindowService messageWindowService;
        private readonly IEventAggregator eventAggregator;
        private readonly ITradeModuleService tradeModuleService;
        private readonly Lazy<ILoginUserManageService> loginUserManageService;
        private readonly DisplayColumnsComposeListConfigItemsContainerModel configItemsContainerModel;
        ...
        public DisplayColumnsComposeListConfigDialogController(IMessageWindowService messageWindowService,
            IEventAggregator eventAggregator,
            ITradeModuleService tradeModuleService,
            Lazy<ILoginUserManageService> loginUserManageService,
            DisplayColumnsComposeListConfigItemsContainerModel configItemsContainerModel,
        ...
        public void Initialize()
        {
            ...
            dialogContentViewModel.ApplyAsGlobalText = "应用到全部组合列表";
        ...
        public void Run()
        {
            configureDialog = messageWindowService.CreateContentCustomWindow(DialogOwner, null, null, true, false, 
                true, "组合列表列配置", dialogContentViewModel.View);
        ...
        {
            if (e.PropertyName == nameof(DisplayColumnsComposeListConfigItemsContainerModel.SelectedConfigItem))
            {
                // 更新操作命令合集对象的选中项
        ...
        private void DisplayColumnsConfigItemsReset(IEnumerable<ListColumnInfo> toDisplayColumnInfos)
        {
            ...
            var allColumnsNames = Enum.GetNames(typeof(SubscribeComposeListDisplayColumn));
            var notDisplayColumnCodes = allColumnsNames
                .Select(n =>
                {
                    Enum.TryParse(n, out SubscribeComposeListDisplayColumn column);
        ...
        private void SaveConfiguration()
        {
            ...
            if (dialogContentViewModel.IsApplyAsGlobal)
            {
                ...
                if (dataRoot != null)
                {
                    dataRoot.GlobalAppliedComposeListDisplayColumns 
                        = configuredColumns.Select(i => (ListColumnInfo)i.Clone()).ToArray();
                }

                // 发布一个事件
                eventAggregator.GetEvent<GlobalApplyComposeListDisplayColumnsEvent>().Publish(configuredColumns);
            }
