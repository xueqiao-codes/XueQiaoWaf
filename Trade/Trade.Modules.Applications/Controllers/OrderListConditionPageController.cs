...
namespace XueQiaoWaf.Trade.Modules.Applications.Controllers
{
    ...
    internal class OrderListConditionPageController : IController
    {
        private readonly IEventAggregator eventAggregator;
        private readonly ExportFactory<DisplayColumnsOrderListConditionConfigDialogController> orderListColumnsConfigDialogCtrlFactory;
        private readonly ExportFactory<RelatedOrderDialogCtrl> relatedOrderDialogCtrlFactory;
        private readonly OrderListConditionPageViewModel pageViewModel;
        ...
        public OrderListConditionPageController(IEventAggregator eventAggregator,
            ExportFactory<DisplayColumnsOrderListConditionConfigDialogController> orderListColumnsConfigDialogCtrlFactory,
            ExportFactory<RelatedOrderDialogCtrl> relatedOrderDialogCtrlFactory,
            OrderListConditionPageViewModel pageViewModel,
        ...
        public void Initialize()
        {
            ...
            var listInitialColumns = Component.AccountContainerComponentDetail.ConditionOrderListComponentDetail.OrderListColumns;
            ...
            eventAggregator.GetEvent<GlobalApplyOrderListConditionColumnsEvent>().Subscribe(ReceiveGlobalApplyOrderListDisplayColumnsEvent);
        ...
        public void Shutdown()
        {
            ...
            eventAggregator.GetEvent<GlobalApplyOrderListConditionColumnsEvent>().Unsubscribe(ReceiveGlobalApplyOrderListDisplayColumnsEvent);
        ...
        {
            var orderListComponentDetail = Component.AccountContainerComponentDetail.ConditionOrderListComponentDetail;
            if (orderListComponentDetail == null)
            {
                orderListComponentDetail = new ConditionOrderListComponentDetail();
                Component.AccountContainerComponentDetail.ConditionOrderListComponentDetail = orderListComponentDetail;
            }

            if (orderListComponentDetail.OrderListColumns?.Any() != true)
            {
                var listInitialColumns = tradeModuleService.TradeWorkspaceDataRoot?.GlobalAppliedOrderListConditionDisplayColumns?.Select(i => i.ColumnCode).ToArray();
                if (listInitialColumns == null)
                {
                    listInitialColumns = TradeWorkspaceDataDisplayHelper.DefaultOrderListConditionDisplayColumns.Select(i => i.GetHashCode()).ToArray();
                }
                orderListComponentDetail.OrderListColumns = listInitialColumns.Select(i => new ListColumnInfo
        ...
        private void ApplyOrderListDisplayColumnsIfNeed(IEnumerable<ListColumnInfo> displayColumnInfos)
        {
            ...
            {
                Component.AccountContainerComponentDetail.ConditionOrderListComponentDetail.OrderListColumns
