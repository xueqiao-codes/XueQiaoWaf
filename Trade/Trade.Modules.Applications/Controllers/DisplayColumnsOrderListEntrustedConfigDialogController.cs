...
namespace XueQiaoWaf.Trade.Modules.Applications.Controllers
{
    ...
    internal class DisplayColumnsOrderListEntrustedConfigDialogController : IController
    {
        private readonly IMessageWindowService messageWindowService;
        private readonly IEventAggregator eventAggregator;
        private readonly ITradeModuleService tradeModuleService;
        private readonly Lazy<ILoginUserManageService> loginUserManageService;
        private readonly DisplayColumnsOrderListEntrustedConfigItemsContainerModel configItemsContainerModel;
        ...
        public DisplayColumnsOrderListEntrustedConfigDialogController(
            IMessageWindowService messageWindowService,
            IEventAggregator eventAggregator,
            ITradeModuleService tradeModuleService,
            Lazy<ILoginUserManageService> loginUserManageService,
            DisplayColumnsOrderListEntrustedConfigItemsContainerModel configItemsContainerModel,
        ...
        public void Initialize()
        {
            ...
            dialogContentViewModel.ApplyAsGlobalText = "应用到全部委托单列表";
        ...
        public void Run()
        {
            configureDialog = messageWindowService.CreateContentCustomWindow(DialogOwner, null, null, true, false, 
                true, "委托单列表列配置", dialogContentViewModel.View);
        ...
        {
            if (e.PropertyName == nameof(DisplayColumnsOrderListEntrustedConfigItemsContainerModel.SelectedConfigItem))
            {
                // 更新操作命令合集对象的选中项
        ...
        private void DisplayColumnsConfigItemsReset(IEnumerable<ListColumnInfo> toDisplayColumnInfos)
        {
            ...
            var allColumnsNames = Enum.GetNames(typeof(OrderListColumn_Entrusted));
            var notDisplayColumnCodes = allColumnsNames
                .Select(n =>
                {
                    Enum.TryParse(n, out OrderListColumn_Entrusted column);
        ...
        private void SaveConfiguration()
        {
            ...
            if (dialogContentViewModel.IsApplyAsGlobal)
            {
                ...
                if (dataRoot != null)
                {
                    dataRoot.GlobalAppliedOrderListEntrustedDisplayColumns
                        = configuredColumns.Select(i => (ListColumnInfo)i.Clone()).ToArray();
                }
                // 发布一个事件
                eventAggregator.GetEvent<GlobalApplyOrderListEntrustedColumnsEvent>().Publish(configuredColumns);
            }
