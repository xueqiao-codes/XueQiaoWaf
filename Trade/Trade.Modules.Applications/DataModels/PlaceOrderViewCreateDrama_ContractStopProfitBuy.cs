...
namespace XueQiaoWaf.Trade.Modules.Applications.DataModels
{
    ...
    /// 合约买入止盈单类型的下单视图创建剧本
    /// </summary>
    public class PlaceOrderViewCreateDrama_ContractStopProfitBuy : PlaceOrderViewCreateDramaBase
    {
        public PlaceOrderViewCreateDrama_ContractStopProfitBuy() 
            : base(ClientPlaceOrderType.BUY_STOP_PROFIT, ClientXQOrderTargetType.CONTRACT_TARGET)
        {
        }
        ...
        protected override void InitDatas()
        {
            ...
            this.OrderTriggerDisplayModel.ConfigTrigger.TriggerOperator = HostingXQConditionTriggerOperator.XQ_OP_LE;

            // 设置可下单时段说明
            this.PlaceOrderAvailableTimeDescription = "任意时段都可下单";

            // 设置买卖按钮区域显隐
            this.ShowBuyButtonAreaView = true;
            this.ShowSellButtonAreaView = false;
        }
        ...
        {
            if (orderDirection != HostingXQTradeDirection.XQ_BUY)
            {
                throw new ArgumentException($"orderDirection must be `{HostingXQTradeDirection.XQ_BUY}`");
            }
            ...
            condition.ConditionAction = new HostingXQConditionAction
            {
                ...
                TradeDirection = HostingXQTradeDirection.XQ_BUY,
                Quantity = orderQuantity,
                Price = triggerOrderPriceTypeValue
            };
            condOrder.Conditions = new List<HostingXQCondition> { condition };

            condOrder.Label = HostingXQConditionOrderLabel.LABEL_STOP_PROFIT_BUY;
